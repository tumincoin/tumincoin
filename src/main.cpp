...
uint256 hashGenesisBlock("0x0bc3eaf21ff55af8930a26ac36e225aacf4d92bb11b920af667429c7456e3dc8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // tumincoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "tumincoin Signed Message:\n";
...
{
    // tumincoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
long double inpower(double dnumber, int64 ipower)
{
...
{
		int64 nFirstTime = 1398639146;
		int64 nNow = GetAdjustedTime();
		
		int64 currMayanMonth = (nNow - nFirstTime) / 86400;	//Mayan circle has 20 days. Calculate current circle since coin release date
	
    int64 nSubsidy = 250 * COIN * inpower(0.98, currMayanMonth);	//Decrease the rewards every Mayan circle by 2%

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 8 * 10 * 60;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // tumincoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // tumincoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0bc3eaf21ff55af8930a26ac36e225aacf4d92bb11b920af667429c7456e3dc8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "27/Apr/2014 Tumincoin release preparation.";	
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1398639146;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1148389;

        if (fTestNet)
        {
            block.nTime    = 1398639146;
            block.nNonce   = 3065312;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2974f615698b04db676547ce4507b42d24bcc547502c4051e24ff2323f54b654"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // tumincoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("tumincoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("tumincoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("tumincoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tumincoin-miner");
    ...
    try { loop {
        ...
        printf("Running tumincoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("tumincoinMiner terminated\n");
        throw;
    }
}
